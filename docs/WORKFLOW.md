...
| 3.3 | Summary runner (uses `summarize.bilingual`) | 双语摘要 | `done` | `src/processing/summary.py` + 36 tests + `scripts/run_summary.py` CLI. Validates summary_en non-empty + summary_zh contains CJK chars + key_numbers list-of-strings. Default skips `track='other'` (archive-only, won't appear in brief). `--include-other`, `--min-importance N`, `--reset`, `--limit`, `--dry-run` flags. |
| 3.4 | Causal-chain generator (uses `causal_chain.five_step`) | 因果链生成 | `done` | `src/processing/causal_chain.py` + 40 tests + `scripts/run_causal_chain.py` CLI. Tier routing: cheap (Haiku 4.5) by default, **strong (Opus 4.7) when `importance >= 70`** per ADR-0010. Validation: all 5 steps × {en, zh non-empty + CJK in zh}, confidence in [0,1], caveats list-of-strings. Persistence: `items.causal_chain` JSON + `items.processed_ts` set to NOW (marks fully processed) + `chain_confidence` / `chain_caveats` merged into `items.meta`. Selection: items where `causal_chain IS NULL AND track IS NOT NULL AND track != 'other' AND summary_en IS NOT NULL`, ordered importance-DESC for early-out brief readiness. |
